...
void reint_float(float& flint)
{
	flint = static_cast<float>(static_cast<int>(std::round(flint)));
}

void re_place(float& flint, const float max_dist)
{
	if (flint > max_dist + 0.001f)
	{
		flint -= (max_dist + max_dist);
	}
	else if (flint < -(max_dist + 0.001f))
	{
		flint += (max_dist + max_dist);
	}
}
...
void image_in_texture(sf::Texture texture, const std::string& image_name)
{
	assert(image_name != "");
		
	if (!texture.loadFromFile(image_name))
	{
		std::cerr << "Could not load " << image_name;
	}
}
...
{	
	...
	std::string m_image_name{"Bit_Mask_1.png"};
	...
	void set_texture()
	{
		...
		{
			std::cerr << "Could not load " << m_image_name;
		}
	}
	
	void set_quads()
	{
		const sf::Vector2u image_sizes{m_texture.getSize()};
		
		// const float image_x{static_cast<float>(image_sizes.x)};
		// const float image_y{static_cast<float>(image_sizes.y)};
		
		const float image_x{1.0f};
		const float image_y{1.0f};
		
		const sf::Vector2f vec_0_0{0.0f, 0.0f};
		const sf::Vector2f vec_x_0{image_x, 0.0f};
		const sf::Vector2f vec_0_y{0.0f, image_y};
		const sf::Vector2f vec_x_y{image_x, image_y};
		
		std::cout << image_x << ":" << image_y << "\n";
		
		for (int count{0}; count < 3; ++count)
		{
			m_quads[count][0].texCoords = vec_0_0;
			m_quads[count][1].texCoords = vec_x_0;
			m_quads[count][2].texCoords = vec_x_y;
			m_quads[count][3].texCoords = vec_0_y;
		}
	}
	
	void reint_posits()
	{
		reint_float(m_central_posit.x);
		reint_float(m_central_posit.y);
		reint_float(m_central_posit.z);
	...
	void replace_posits()
	{
		re_place(m_central_posit.x, m_max_dist);
		re_place(m_central_posit.y, m_max_dist);
		re_place(m_central_posit.z, m_max_dist);
	...
	void show_rects(sf::RenderWindow& window)
	{
		if (m_sighted)
		{
			...
			window.draw(m_quads[first], &texture);
			window.draw(m_quads[middle], &texture);
			window.draw(m_quads[last], &texture);
		}	
	}
	...
	cuboid(const sf::Color& color, const sf::Vector3f& central_posit, const float max_dist,
		   const float side_length, const float delta_dist, const float sin_phi, const float cos_phi)
		   : m_color(color), m_central_posit(central_posit), m_max_dist(max_dist), m_side_length(side_length),
		   m_delta_dist(delta_dist), m_sin_phi(sin_phi), m_cos_phi(cos_phi),
		   m_abs_posits(), m_abs_squares(), m_texture()
	{
		setup_posits();
		setup_squares();
		place_posits();
...
void show_kubes(std::vector <cuboid>& vis_kubes, sf::RenderWindow& window, const int vis_kube_numbers)
{
	...
	for (int count{0}; count < vis_kube_numbers; ++count)
	{
		...
		// vis_kubes[count].set_texture();
		// vis_kubes[count].set_quads();
...
int main()
{
	const std::string program_name{"Cuboid V0.9"};
	assert(program_name != "");
	...
	const float max_dist{static_cast<float>(max_count + barrier)};
	...
	for (int count{0}; count < kube_numbers; ++count)
	{
		kubes.push_back(cuboid (kolors[count], central_posits[count], max_dist, side_length, delta_dist, sin_phi, cos_phi));
	}
