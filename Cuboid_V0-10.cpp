...
sf::Vector2f three_to_two(const sf::Vector3f& position)
{
	...
	posit_x = 1/posit_x;	
	return sf::Vector2f (mult*position.y*posit_x, -mult*position.z*posit_x);	
}
...
class cuboid
{
	...
	bool free_move(const sf::Vector3f& key_move)
	{
		if (middle_square() < 1.1f)
		{
			const int key_x{static_cast<int>(key_move.x)};
			const int key_y{static_cast<int>(key_move.y)};
			const int key_z{static_cast<int>(key_move.z)};
			
			const int pos_x{static_cast<int>(m_central_posit.x)};
			const int pos_y{static_cast<int>(m_central_posit.y)};
			const int pos_z{static_cast<int>(m_central_posit.z)};
			
			if (((pos_x != 0) && (pos_x == key_x)) ||
				((pos_y != 0) && (pos_y == key_y)) ||
				((pos_z != 0) && (pos_z == key_z)))
			{
				return false;
...
int main()
{
	const std::string program_name{"Cuboid V0.10"};
	assert(program_name != "");
	
	const float pi{4.0f*atan(1.0f)};
	
	const float divisions{40.0f};
		...
		{		
			...
			if (!moving)
			{
				...
				if (moving)
				{
					for (int count{0}; count < kube_numbers; ++count)
					{
						if (!kubes[count].free_move(key_move))
						{
							moving = false;
						}
					}				
